...
class LITagPositionCache {
    private final HashMap<String, LITagSearchResult> tag_positions = new HashMap<>();
    ...
    LITagSearchResult search(String tag) {
        return tag_positions.get(tag);
    }
    void cache_search(String tag, LITagSearchResult sr) {
    ...
    void remove_cache_entry(String tag) {
        LITagSearchResult tag_sr = tag_positions.remove(tag);
