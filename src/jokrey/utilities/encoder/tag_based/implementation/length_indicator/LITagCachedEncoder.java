...
//        fill_search_cache();  // don't do that. cache on demand - much faster
    ...
    @Override protected LITagSearchResult search(String tag) {
        LITagSearchResult cache_sr = cache.search(tag);
...
//                throw new IllegalStateException("should not happen and this sanity check can be removed as soon as tests show that it doesn't");
        ...
        LIe.Position search_pos = lie.reset();
        long last_read_pointer = 0;
        String dec_tag;
        long entry_length;
        while((dec_tag = getTag(lie.li_decode(search_pos))) != null &&
                ((entry_length = lie.li_skip(search_pos)) != -1)) {

            long entry_end_index = lie.get(search_pos);
            LITagSearchResult sr = new LITagSearchResult(entry_end_index-entry_length, entry_end_index, last_read_pointer);
            ...
            last_read_pointer = lie.get(search_pos);

            if(tag.equals(dec_tag)) {
                if(sr.entry_end_index == lie.storageSize()) //was last entry
...
//changed wrapping:::
    ...
    @Override public LITagCachedEncoder<SF> addEntry_nocheck(String tag, SF arr) {
        long old_length = lie.storageSize();
    ...
    void cache_add(String tag, long old_length_before_add) {
        ...
        LIe.Position fake_search_pos = lie.from(old_length_before_add);
        lie.li_skip(fake_search_pos); //skip tag
        long entry_length = lie.li_skip(fake_search_pos); //skip entry
        long entry_end_index = lie.get(fake_search_pos);
        LITagSearchResult added_sr = new LITagSearchResult(entry_end_index-entry_length, entry_end_index, old_length_before_add);
    ...
    @Override public SF deleteEntry(String tag) {
        LITagSearchResult sr = search(tag); //already cached
        if(sr==null) return null;
        SF entry = lie.getStorageSystem().sub(sr.entry_start_index, sr.entry_end_index);
    ...
    @Override public String[] getTags() {
        if(cache.is_fully_cached()) {
            ...
            while ((dec_tag = getTag(lie.li_decode(search_pos))) != null &&
                ...
                long entry_end_index = lie.get(search_pos);
                LITagSearchResult sr = new LITagSearchResult(entry_end_index - entry_length, entry_end_index, last_raw_read_pointer);

                cache.cache_search(dec_tag, sr);

                last_raw_read_pointer = lie.get(search_pos);
